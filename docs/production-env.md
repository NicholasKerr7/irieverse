...
For production scale, set `ROUTING_API_BASE_URL` to your own OSRM-compatible service or a paid routing provider proxy. `ROUTING_API_TIMEOUT_MS` keeps slow provider calls from holding the map open, while `ROUTING_PROVIDER_COOLDOWN_SECONDS` keeps repeated provider failure logs from getting noisy. If road routing fails, the map falls back to the local preview route instead of breaking and keeps the external Google Maps handoff available for full turn-by-turn navigation and traffic.
