...
export function BottomNav({ activeTab, onChange }: BottomNavProps) {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-800/90 bg-slate-950/95 px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-2 backdrop-blur-xl">
        ...
        {TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl text-[0.68rem] font-medium transition ${
                isActive
